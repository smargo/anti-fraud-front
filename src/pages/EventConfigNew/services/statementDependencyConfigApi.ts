...
export const queryStatementDependencies = async (params: PaginationParams): Promise<ApiResponse<StatementDependencyItem[]>> => {
  const page: { data: StatementDependencyItem[]; total: number; success: boolean } = await request('/api/statement-dependency/list', {
    method: 'GET',
    params,
  });
  return {
...
export const queryStatementDependenciesWithNames = async (params: PaginationParams): Promise<ApiResponse<StatementDependencyItem[]>> => {
  const page: { data: StatementDependencyItem[]; total: number; success: boolean } = await request('/api/statement-dependency/list-with-names', {
    method: 'GET',
    params,
  });
  return {
