...
import type { StatementDependencyItem, StatementDependencyFormValues, StatementDependencyConfigState } from '../types';

export const useStatementDependencyConfig = (eventNo: string, versionCode?: string, externalActionRef?: React.RefObject<any>) => {
  // 语句依赖配置状态
  ...
  // 加载语句依赖列表
  const loadStatementDependencies = useCallback(async (params: any = {}) => {
    try {
      setLoading(true);
      const searchParams = {
        eventNo,
      ...
      if (response.code === 'SUCCESS') {
        setStatementDependencies(response.records || []);
        return {
        ...
        message.error(response.message || '加载语句依赖列表失败');
  ...
  // 创建语句依赖
  const handleCreateStatementDependency = useCallback(async (values: StatementDependencyFormValues) => {
    try {
  ...
  // 更新语句依赖
  const handleUpdateStatementDependency = useCallback(async (values: StatementDependencyFormValues) => {
