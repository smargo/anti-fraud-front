 ...
 * 选择最佳版本（生效版本 > 草稿版本）
 */
export const selectBestVersion = (
  versionHistory: EventConfigVersion[],
): EventConfigVersion | null => {
 ...
 * 选择指定版本
 */
export const selectSelectedVersion = (
  versionHistory: EventConfigVersion[],
  ...
  // 优先选择指定版本
  const selectedVersion = versionHistory.find((v) => v.versionCode === selectedVersionNo);
  if (selectedVersion) {
 ...
 * 检查是否应该显示编辑界面
 */
export const shouldShowEditInterface = (configEventLoadProp: any, isReadOnly: boolean): boolean => {
  return (
    configEventLoadProp &&
