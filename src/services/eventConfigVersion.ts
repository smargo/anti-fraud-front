...
export const versionApi = {
  ...
  createVersion: async (data: CreateVersionRequest): Promise<EventConfigVersion> => {
    const response: ApiResponse<EventConfigVersion> = await request('/api/event-config-version', { method: 'POST', data });
  ...
  getCurrentVersion: async (eventNo: string): Promise<EventConfigVersion | null> => {
    const response: ApiResponse<EventConfigVersion> = await request(`/api/event-config-version/current/${eventNo}`);
    if (response.code === 'SUCCESS') {
      return response.data;
    } else {
  ...
  getVersionHistory: async (eventNo: string): Promise<EventConfigVersion[]> => {
    const response: ApiResponse<EventConfigVersion[]> = await request(`/api/event-config-version/history/${eventNo}`);
    if (response.code === 'SUCCESS') {
      return response.data;
    } else {
  ...
  // 分页查询版本历史（支持搜索）
  getVersionHistoryPage: async (eventNo: string, params: {
    versionCode?: string;
    versionDesc?: string;
    status?: string;
    current?: number;
  ...
  activateVersion: async (versionId: string): Promise<void> => {
    const response: ApiResponse<void> = await request(`/api/event-config-version/${versionId}/activate`, { method: 'POST' });
    if (response.code !== 'SUCCESS') {
  ...
  copyVersion: async (versionId: string, newVersionCode: string): Promise<EventConfigVersion> => {
    const response: ApiResponse<EventConfigVersion> = await request(`/api/event-config-version/${versionId}/copy?newVersionCode=${encodeURIComponent(newVersionCode)}`, { method: 'POST' });
    if (response.code === 'SUCCESS') {
      return response.data;
    } else {
  ...
  rollbackToVersion: async (versionId: string): Promise<void> => {
    const response: ApiResponse<void> = await request(`/api/event-config-version/${versionId}/rollback`, { method: 'POST' });
    if (response.code !== 'SUCCESS') {
  ...
  submitForApproval: async (versionId: string): Promise<void> => {
    const response: ApiResponse<void> = await request(`/api/event-config-version/${versionId}/submit`, { method: 'POST' });
    if (response.code !== 'SUCCESS') {
  ...
  approveVersion: async (versionId: string, approver: string): Promise<void> => {
    const response: ApiResponse<void> = await request(`/api/event-config-version/${versionId}/approve?approver=${encodeURIComponent(approver)}`, { method: 'POST' });
    if (response.code !== 'SUCCESS') {
  ...
  rejectVersion: async (versionId: string, reason: string): Promise<void> => {
    const response: ApiResponse<void> = await request(`/api/event-config-version/${versionId}/reject?reason=${encodeURIComponent(reason)}`, { method: 'POST' });
    if (response.code !== 'SUCCESS') {
  ...
  getChangeLogs: async (versionId: string): Promise<ConfigChangeLog[]> => {
    const response: ApiResponse<ConfigChangeLog[]> = await request(`/api/event-config-version/${versionId}/change-logs`);
    if (response.code === 'SUCCESS') {
      return response.data;
    } else {
  ...
  getChangeStatistics: async (versionId: string): Promise<any[]> => {
    const response: ApiResponse<any[]> = await request(`/api/event-config-version/${versionId}/change-statistics`);
    if (response.code === 'SUCCESS') {
      return response.data;
    } else {
  ...
  compareVersions: async (versionId1: string, versionId2: string): Promise<any> => {
    const response: ApiResponse<any> = await request(`/api/event-config-version/compare?versionId1=${versionId1}&versionId2=${versionId2}`);
    if (response.code === 'SUCCESS') {
      return response.data;
    } else {
  ...
  deleteDraftVersion: async (versionId: string): Promise<boolean> => {
    const response: ApiResponse<boolean> = await request(`/api/event-config-version/${versionId}`, { method: 'DELETE' });
    if (response.code === 'SUCCESS') {
      return response.data;
    } else {
...
export const getVersionInfo = async (eventNo: string): Promise<EventConfigVersionInfo> => {
  try {
    ...
    const isPublished = currentVersion?.status === 'ACTIVE' || currentVersion?.status === 'APPROVED';
...
export const createDraft = async (eventNo: string, versionCode: string, versionDesc: string): Promise<EventConfigVersion> => {
...
export const rollbackToVersion = async (versionId: string, eventNo: string): Promise<EventConfigVersion> => {
...
// 更新版本信息
export const updateVersion = async (versionId: string, data: Partial<EventConfigVersion>): Promise<EventConfigVersion> => {
  const response: ApiResponse<EventConfigVersion> = await request(`/api/event-config-version/${versionId}`, { 
    method: 'PUT', 
    data 
  });
  if (response.code === 'SUCCESS') {
